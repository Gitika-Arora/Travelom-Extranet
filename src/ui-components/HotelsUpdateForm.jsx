...
export default function HotelsUpdateForm(props) {
  const {
    id: idProp,
    hotels: hotelsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = hotelsRecord
      ? { ...initialValues, ...hotelsRecord }
      : initialValues;
    setUserId(cleanValues.userId);
    setHotelDescriptiveContents(
      typeof cleanValues.hotelDescriptiveContents === "string" ||
        cleanValues.hotelDescriptiveContents === null
        ? cleanValues.hotelDescriptiveContents
        : JSON.stringify(cleanValues.hotelDescriptiveContents)
    );
    setBrandCode(cleanValues.brandCode);
    setHotelCode(cleanValues.hotelCode);
    setHotelName(cleanValues.hotelName);
    setAddress(cleanValues.address);
    setCity(cleanValues.city);
    setPostalCode(cleanValues.postalCode);
    setStateCode(cleanValues.stateCode);
    setCountyCode(cleanValues.countyCode);
    setHotelUpdatedInExtranet(cleanValues.hotelUpdatedInExtranet);
  ...
  const [hotelsRecord, setHotelsRecord] = React.useState(hotelsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getHotels.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getHotels
        : hotelsModelProp;
      setHotelsRecord(record);
    };
    queryData();
  }, [idProp, hotelsModelProp]);
  React.useEffect(resetStateValues, [hotelsRecord]);
  const validations = {
    userId: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          userId: userId ?? null,
          hotelDescriptiveContents: hotelDescriptiveContents ?? null,
          ...
          hotelUpdatedInExtranet: hotelUpdatedInExtranet ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateHotels.replaceAll("__typename", ""),
            variables: {
              input: {
                id: hotelsRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "HotelsUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      <TextAreaField
        ...
        isRequired={false}
        isReadOnly={false}
        value={hotelDescriptiveContents}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || hotelsModelProp)}
