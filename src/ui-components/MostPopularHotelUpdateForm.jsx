...
import { getMostPopularHotel } from "../graphql/queries";
import { updateMostPopularHotel } from "../graphql/mutations";
const client = generateClient();
export default function MostPopularHotelUpdateForm(props) {
  const {
    id: idProp,
    mostPopularHotel: mostPopularHotelModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mostPopularHotelRecord
      ? { ...initialValues, ...mostPopularHotelRecord }
      : initialValues;
    setBrand(cleanValues.brand);
    setPid(cleanValues.pid);
    setPropName(cleanValues.propName);
  ...
  const [mostPopularHotelRecord, setMostPopularHotelRecord] = React.useState(
    mostPopularHotelModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getMostPopularHotel.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMostPopularHotel
        : mostPopularHotelModelProp;
      setMostPopularHotelRecord(record);
    };
    queryData();
  }, [idProp, mostPopularHotelModelProp]);
  React.useEffect(resetStateValues, [mostPopularHotelRecord]);
  const validations = {
    brand: [],
    pid: [],
    propName: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateMostPopularHotel.replaceAll("__typename", ""),
            variables: {
              input: {
                id: mostPopularHotelRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MostPopularHotelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Brand"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mostPopularHotelModelProp)}
